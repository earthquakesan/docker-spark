...
build:
	docker build --no-cache -t bde2020/spark-base:$(current_branch) ./base
	docker build --no-cache -t bde2020/spark-master:$(current_branch) ./master
	docker build --no-cache -t bde2020/spark-worker:$(current_branch) ./worker
	docker build --no-cache -t bde2020/spark-submit:$(current_branch) ./submit
